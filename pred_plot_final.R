...
generate_pred_plot <- function(data, method = "LM", baseTemp = NULL, windowSize = 5, showMA = "none", showData = TRUE) {
 

# Data prep ---------------------------------------------------------------

   # make columns for confidence intervals
  data$Lower <- NA
  data$Upper <- NA
...
# Moving average and Predictions ------------------------------------------
  ...
  # adjust data based on  selected moving average
  ...
  if (method == "LM") {
    ...
    future_years$Lower <- pred -  sd(pred) # place holder CI (not sure which one to actually use with GLM)
    future_years$Upper <- pred +  sd(pred) # place holder CI (not sure which one to actually use with GLM)
    smooth_method <- "glm"
  } else if (method == "GAM") {
...
# Plot --------------------------------------------------------------------
  ...
  # blue rectangle showing sleceted years 
  if (!is.null(baseTemp)) {
    plot <- plot +
...
# example usage
# df <- read.csv("global_temps.csv", sep = ',')
# generate_pred_plot(data = df,
#                    method = "GAM",
#                    windowSize = 5, 
#                    showMA = "CMA", 
#                    baseTemp = c(1900, 1950),
#                    showData = T)
